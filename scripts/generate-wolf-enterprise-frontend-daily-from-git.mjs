#!/usr/bin/env node
/**
 * Reads git log from wolf-enterprise-frontend origin/main and writes
 * changelog/wolf-enterprise-frontend/YYYY-MM-DD.mdx per author date.
 *
 * Usage:
 *   WOLF_ENTERPRISE_FRONTEND_ROOT=../wolf-enterprise-frontend node scripts/generate-wolf-enterprise-frontend-daily-from-git.mjs
 */
...
const REPO = path.resolve(
  process.env.WOLF_ENTERPRISE_FRONTEND_ROOT ??
    path.join(ROOT, "..", "wolf-enterprise-frontend")
);
const OUT_DIR = path.join(ROOT, "changelog", "wolf-enterprise-frontend");

const SINCE = process.env.CHANGELOG_SINCE ?? "2026-01-20";
const UNTIL = process.env.CHANGELOG_UNTIL ?? "2026-03-21";

const TITLE_PREFIX = "Platform";
...
function gitLog() {
  try {
    ...
    console.error(
      "git log failed — set WOLF_ENTERPRISE_FRONTEND_ROOT or clone wolf-enterprise-frontend next to wolf-changelog."
    );
    throw e;
  }
}
...
function shouldSkipSubject(s) {
  ...
  if (/detailed loggin|console for the user permissions/i.test(low)) return true;
  return false;
}
